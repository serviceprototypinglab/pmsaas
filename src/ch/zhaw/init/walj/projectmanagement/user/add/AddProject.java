...
public class AddProject extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println(HTMLHeader.getInstance().printHeader("New Project", "../", "New Project", script, "", false)
				...
				+ "<p class=\"small-12 columns\">* required</p>"
				+ "</div>"
				+ "</form>"
				+ "</div>"
				+ "</section>"
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Workpackage> workpackages = new ArrayList<>();
		...
		for (int i = 0; i < taskName.length; i++) {
			tasks.add(new Task(i, taskWP[i], taskName[i], taskStart[i], pStart, taskEnd[i], Integer.parseInt(taskPM[i]), Double.parseDouble(taskBudget[i]), new ArrayList<>()));
		}
		...
		if (flag == 0) {
			try {
				...
				String message = "<div class=\"callout success\">"
						...
						+ "<a href=\"/Projects/Overview/Project?id=" + pID + "\">Click here to go to the project overview</a>"
						+ "</div>";
